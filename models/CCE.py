...
MAT_params = {
    # pre-defined MAT parameters for loading pretrained model weights
    'd_atom': 28,
    'd_model': 1024,
    'N': 8,
    'h': 16,
...
class CCE(nn.Module):
    def __init__(self, d_gex, d_mat=1024, d_dosetime=2, dropout=0.1, 
                 bias=False, MAT_params=MAT_params):
        super(CCE, self).__init__()
        self.comp_proj=nn.Sequential(
            nn.Linear(d_mat + 2*d_dosetime, d_gex, bias=bias),
            nn.BatchNorm1d(d_gex),
            nn.ReLU(),
            nn.Dropout(p=dropout)
        )

        # Linear layer for encoding dose and time
        self.time_enc=nn.Linear(1,d_dosetime)
        self.dose_enc=nn.Linear(1,d_dosetime)
        self.MAT = make_MAT(**MAT_params)

    def compound_dimreduction(self,comp,mask,dose,time):
        dose=self.dose_enc(dose)
        ...
        comp=comp/mask_sum
        comp=torch.cat([comp,dose,time],dim=1)
        return self.comp_proj(comp)        
    
    def forward(self, node_feat, mask, adj_matrix, distance_matrix, dose, time):
        mat_comp = self.MAT(node_feat,mask, adj_matrix,distance_matrix,None)
        cs_comp = self.compound_dimreduction(mat_comp,mask,dose,time)
        return cs_comp

def make_MAT(d_atom, N=2, d_model=128, h=8, dropout=0.1, 
...
class SublayerConnection(nn.Module):
    ...
    def __init__(self, size, dropout, scale_norm, use_adapter):
        ...
        self.adapter = None # Adapter is not used in original MAT implementation
